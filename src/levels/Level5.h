#ifndef LEVEL5_GAMEPLAY_H
#define LEVEL5_GAMEPLAY_H
...
static bool isSolidTileLevel5(int row, int col) {
    if (row < 0 || row >= MAP_ROWS || col < 0 || col >= MAP_COLS) return false;
    int t = LEVEL5_MAP[row][col];
    return (t == 1 || t == 2 || t == 4);
}
...
static void initLevel5() {
    ...
    for (int r = 0; r < MAP_ROWS; ++r) {
        for (int c = 0; c < MAP_COLS; ++c) {
            if (LEVEL5_MAP[r][c] == 2) { doorRow = r; doorCol = c; break; }
        }
        if (doorRow != -1) break;
    }
    if (doorRow == -1) { doorRow = 2; doorCol = 38; }
...
static void runGameLevel5() {
    ...
    while (currentState == PLAYING && running) {
        ...
        if (!paused) {
            ...
            if (player.vx > 0.0f) {
                ...
                for (int r = topY / TILE_SIZE; r <= bottomY / TILE_SIZE; ++r)
                    if (isSolidTileLevel5(r, col)) { hit = true; break; }
            ...
            if (player.vy > 0.0f) {
                ...
                for (int c = leftX / TILE_SIZE; c <= rightX / TILE_SIZE; ++c)
                    if (isSolidTileLevel5(row, c)) { hit = true; break; }
        ...
        drawPlayer(player);
        drawPlayerHP(player);
        drawLevelHUD(5, keysCollected, GAME_KEYS_REQUIRED, paused);
